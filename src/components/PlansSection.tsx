...
export const PlansSection: React.FC<PlansSectionProps> = ({ settings, onSubscribed }) => {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Gancho Mercado Pago: quando o checkout existir, redirecionamos.
      if (res.checkoutUrl) {
    ...
    <section id="planos" className="py-16 bg-[#1A1A1A] text-white">
          ...
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {FEATURES.map((f) => {
              ...
              return (
                <div
                  key={f.title}
                  className="flex items-start space-x-3 p-4 rounded-2xl bg-zinc-900 border border-zinc-800"
                >
            ...
            <div>
              ...
              <div className="mt-2 flex items-end space-x-1">
                <span className="text-4xl font-extrabold text-white">
                  {formatCurrency(plan?.priceMonthly || 89.9)}
                </span>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white text-slate-900 rounded-3xl max-w-md w-full p-6 relative shadow-2xl">
            <button
            ...
            {done ? (
              <div className="text-center py-8 space-y-3">
                ...
                <h3 className="text-xl font-bold text-slate-900">Assinatura registrada!</h3>
                <p className="text-xs text-slate-500 max-w-xs mx-auto">
                  Recebemos sua solicitação do <strong>{plan?.name || 'Plano Pro'}</strong>. Em
                  breve você poderá concluir o pagamento via Mercado Pago. Entraremos em contato pelo
                  e-mail informado.
                </p>
                <button
                  onClick={resetAndClose}
                  className="mt-2 px-5 py-2.5 rounded-full bg-[#1A1A1A] hover:bg-amber-600 text-white font-bold text-xs transition-colors"
                >
                  Concluir
                </button>
              </div>
              ...
              <form onSubmit={handleSubscribe} className="space-y-4">
                <div className="text-center space-y-1">
                  ...
                  <p className="text-xs text-slate-500">
                    {formatCurrency(plan?.priceMonthly || 89.9)}/mês — preencha para iniciar.
                  </p>
                </div>
                ...
                >
                  {loading ? 'Enviando...' : 'Confirmar Contratação'}
                </button>
              </form>
            )}
          </div>
